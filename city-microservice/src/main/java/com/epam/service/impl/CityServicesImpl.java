...
public class CityServicesImpl implements CityServices {
	...
	public CityResponseDto getCityById(String cityId) throws SQLException {
		...
		try {
			city = CityMapper.MAPPER.toCityDto(cityRepository.findByCityId(cityId));
		} catch (Exception e) {
	...
	public CityResponseDto insert(CityRequestDto dto) throws SQLException {
		CityResponseDto city = CityMapper.MAPPER.toCityDto(cityRepository.save(CityMapper.MAPPER.toCityEntity(dto)));
		return city;
	}
